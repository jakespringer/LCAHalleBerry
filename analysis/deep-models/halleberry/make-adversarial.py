...
for target_model_name in ['resnet50', 'inceptionv3', 'vgg16', 'mobilenetv2', 'densenet121']:
    ...
    X_benign, X_adversarial, X_noisy = model.make_adversarial_and_noisy_batch(X_test, y_test, eps=4.0, num_iter=5, batch_size=25)
    ...
    def write_image(info, directory):
        ...
        if not os.path.exists('dataset-{}/{}/{}'.format(target_model_name, directory, np.argmax(y) + 1)):
            os.makedirs('dataset-{}/{}/{}'.format(target_model_name, directory, np.argmax(y) + 1))
    
        filename = 'dataset-{}/{}/{}/{}.png'.format(target_model_name, directory, np.argmax(y) + 1, i)
        open('dataset-' + target_model_name + '/faces_' + directory + '.txt', 'a').write(os.path.abspath(filename) + '\n')
        imageio.imwrite(filename, x)
        print('Wrote', filename)
    
    for i in enumerate(zip((X_benign), y_test)):
        write_image(i, 'benign')
    
    for i in enumerate(zip((X_adversarial), y_test)):
        write_image(i, 'adversarial')
    
    for i in enumerate(zip((X_noisy), y_test)):
        write_image(i, 'noisy')
